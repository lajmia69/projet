...
// ─── Email-exists popup ───────────────────────────────────────────────────────
function EmailExistsDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
	return (
		<Dialog
			open={open}
			onClose={onClose}
				...
				<Box sx={{ display: 'flex', gap: 1.5 }}>
					<Box
						...
						sx={{
							...
							'&:hover': { background: 'rgba(232,228,218,0.12)' },
						}}
					>
						Cancel
					</Box>
...
function SignUpPageView() {
	...
	// Listen for error params — your sign-up API should redirect with
	// ?error=EmailAlreadyExists (or OAuthAccountNotLinked for OAuth flow)
	useEffect(() => {
		...
		if (
			...
			router.replace(`/sign-up${clean ? `?${clean}` : ''}`, { scroll: false });
		}
	}, [searchParams, router]);
	...
	return (
		...
		>
			...
			{/* ── EMAIL EXISTS DIALOG ── */}
			<EmailExistsDialog open={emailExistsOpen} onClose={() => setEmailExistsOpen(false)} />
		</Box>
	);
}
