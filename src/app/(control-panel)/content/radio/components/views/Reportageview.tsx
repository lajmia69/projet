	...
	return (
		<>
					...
					<div className="flex items-center justify-between gap-2 pt-0.5">
						{reportage.created_by?.full_name && (
							<div className="flex items-center gap-1.5 min-w-0">
								<FuseSvgIcon size={13} sx={{ color: 'rgba(26,46,56,0.3)', flexShrink: 0 }}>lucide:newspaper</FuseSvgIcon>
			...
			<Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} PaperProps={{ sx: { borderRadius: '16px', minWidth: 320 } }}>
				<DialogTitle sx={{ fontWeight: 700, fontSize: '1rem', pb: 1 }}>Delete reportage?</DialogTitle>
				<DialogContent>
				...
				<DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
					...
					<Button onClick={() => { onDelete(reportage.id); setConfirmOpen(false); }} variant="contained" color="error" size="small" sx={{ borderRadius: '9px', textTransform: 'none', fontWeight: 600 }}>Delete</Button>
				</DialogActions>
			</Dialog>
		</>
	);
					...
					{filteredData.length > 0 ? (
						<motion.div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4" variants={cardContainer} initial="hidden" animate="show">
							{filteredData.map((reportage) => (
								<motion.div variants={cardItem} key={reportage.id}>
									<ReportageCard reportage={reportage} onDelete={(id) => deleteReportage(id)} />
								</motion.div>
							))}
						</motion.div>
					) : (
						<div className="flex flex-1 items-center justify-center py-20">
							<div className="flex flex-col items-center gap-3">
								...
								<Typography color="text.secondary" variant="h6">No reportages found</Typography>
