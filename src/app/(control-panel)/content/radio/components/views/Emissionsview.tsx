		...
		<>
			<Card
				sx={(theme) => ({
					...
					border: theme.palette.mode === 'dark' ? '1px solid rgba(14,168,176,0.25)' : '1px solid rgba(14,168,176,0.22)',
					...
					'&:hover': {
						...
						borderColor: theme.palette.mode === 'dark' ? 'rgba(14,168,176,0.5)' : 'rgba(14,168,176,0.5)',
						boxShadow: theme.palette.mode === 'dark'
					...
					<div className="flex items-center gap-3 flex-wrap">
						...
						{emission.is_published && (
							<Chip label="On Air" size="small" sx={() => ({
								...
								color: '#fff',
								backgroundColor: 'rgba(14,168,176,0.35)',
								border: '1px solid rgba(14,168,176,0.6)',
							})} />
						)}
					</div>
					...
					<div className="flex items-center justify-between gap-2 pt-0.5">
						{emission.created_by?.full_name && (
							<div className="flex items-center gap-1.5 min-w-0">
								<FuseSvgIcon size={13} sx={{ color: 'rgba(255,255,255,0.3)', flexShrink: 0 }}>lucide:radio</FuseSvgIcon>
						...
						<Button component={Link} to={`/content/radio/emissions/${emission.id}`} size="small" variant="contained"
							sx={() => ({
								...
								background: 'linear-gradient(135deg, #0EA8B0, #1DC98A)', color: '#0D1A47',
								boxShadow: '0 0 14px rgba(14,168,176,0.5)',
								'&:hover': { background: 'linear-gradient(135deg, #1DC98A, #0EA8B0)', transform: 'scale(1.04)', boxShadow: '0 0 20px rgba(14,168,176,0.65)' },
							})}
		...
		<Root
			...
			header={
				<div style={{
					...
					<div style={{ position: 'absolute', top: '-100px', left: '-120px', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(14,168,176,0.22) 0%, transparent 65%)' }} />
					{/* Teal accent orb — right */}
					...
					<div className="relative flex flex-col items-center justify-center px-6 text-center" style={{ zIndex: 1 }}>
						...
						{emissions?.count != null && (
							<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.24, duration: 0.4 } }} className="mt-5">
								<div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '4px 14px', borderRadius: '999px', border: '1px solid rgba(14,168,176,0.35)', backgroundColor: 'rgba(14,168,176,0.12)' }}>
