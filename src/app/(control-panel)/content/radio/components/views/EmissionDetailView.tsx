...
type Props = { emissionId: string };

function EmissionDetailView({ emissionId }: Props) {
	...
	if (!emission) {
		return (
			<Root
				...
				header={<div className="p-6"><Typography variant="h6">Emission not found</Typography></div>}
				content={
					<div className="flex flex-1 items-center justify-center py-16">
						<Typography color="text.disabled">This emission could not be loaded.</Typography>
					</div>
				}
			/>
		);
						...
						{emission.emission_type?.name && (
							<Chip label={emission.emission_type.name} size="small"
								sx={(theme) => ({
									...
									backgroundColor: theme.palette.mode === 'dark' ? 'rgba(245,158,11,0.18)' : 'rgba(245,158,11,0.1)',
					...
					<div className="flex items-center gap-4 mt-1 flex-wrap">
						...
						{emission.created_by?.full_name && (
							<div className="flex items-center gap-1.5">
								...
								<Typography className="text-sm" color="text.secondary">{emission.created_by.full_name}</Typography>
							</div>
						)}
					</div>
				</div>
			...
			content={
				<div className="mx-auto flex w-full flex-1 flex-col p-4">
					{emission.description && (
						<>
							<Typography color="text.secondary" className="text-sm mb-4">{emission.description}</Typography>
