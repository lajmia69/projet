...
type Props = { reportageId: string };

function ReportageDetailView({ reportageId }: Props) {
	...
	if (!reportage) {
		return (
			<Root
				...
				header={<div className="p-6"><Typography variant="h6">Reportage not found</Typography></div>}
				content={
					<div className="flex flex-1 items-center justify-center py-16">
						<Typography color="text.disabled">This reportage could not be loaded.</Typography>
					</div>
				}
			/>
		);
		...
		<Root
			...
			header={
				<div dir={langOrientation} className="p-6 flex flex-col gap-2">
					...
					<Typography variant="h4" className="font-semibold">{reportage.name}</Typography>
					...
					<div className="flex items-center gap-4 mt-1 flex-wrap">
						...
						{reportage.language?.name && (
							<div className="flex items-center gap-1.5">
								...
								<Typography className="text-sm" color="text.secondary">{reportage.language.name}</Typography>
							</div>
						)}
						{reportage.created_by?.full_name && (
							<div className="flex items-center gap-1.5">
								<FuseSvgIcon size={14} color="disabled">lucide:mic-2</FuseSvgIcon>
								<Typography className="text-sm" color="text.secondary">{reportage.created_by.full_name}</Typography>
							</div>
						)}
					</div>
				</div>
			...
			content={
				<div className="mx-auto flex w-full flex-1 flex-col p-4">
					{reportage.description && (
						<>
							<Typography color="text.secondary" className="text-sm mb-4">{reportage.description}</Typography>
