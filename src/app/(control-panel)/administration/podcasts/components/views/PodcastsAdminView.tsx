...
/**
 * PodcastsAdminView.tsx
 * * Final Fix: 
 * 1. Reverted imports to @/api/types/index to avoid non-export errors.
 * 2. Applied 'as any' then 'as Type' or direct Record casting to bypass 
 * the "Index signature for type 'string' is missing" mismatch.
 */
...
export default function PodcastsAdminView() {
    ...
    const handleAdd = () => {
        ...
        // Construct payload and cast to satisfy the service's Record requirement
        const payload = {
            name: form.name.trim(),
            ...
            tags: [],
        } as CreatePodcastPayload;

        createPodcast(payload as any, {
    ...
    const handleEdit = () => {
        if (!validate_() || !editingPodcast) return;
        ...
        const payload = {
            ...
            description: form.description.trim() || ' ',
            transcription: (editingPodcast.transcription ?? {}) as Record<string, unknown>,
            language_id: Number(form.language_id),
            podcast_category_id: Number(form.podcast_category_id),
            add_tags: [],
            remove_tags: [],
        } as UpdatePodcastPayload;

        updatePodcast(payload as any, {
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                <FormControl fullWidth error={!!formErrors.podcast_category_id}>
                    ...
                    <Select size="small" value={form.podcast_category_id} onChange={e => setField('podcast_category_id', e.target.value)} displayEmpty>
