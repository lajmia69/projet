...
export default function CultureActivityTypesAdminView() {
  ...
  // Mutations
  const { mutate: createType, isPending: isCreating } = useCreateCulturalActivityType() as any;
  const { mutate: updateType, isPending: isUpdating } = useUpdateCulturalActivityType() as any;
  const { mutate: deleteType, isPending: isDeleting } = useDeleteCulturalActivityType() as any;

  // Dialog state
  const [addOpen, setAddOpen] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<number | null>(null);
  const [editingRow, setEditingRow] = useState<CulturalActivityType | null>(null);
  ...
  const openEdit = (row: CulturalActivityType) => {
    setEditingRow(row);
  ...
  const handleAdd = () => {
    ...
    createType(
      { name: form.name.trim(), description: form.description.trim() },
      {
        ...
        onError: (err: any) => console.error('[ActivityTypes] create failed', err),
      },
    );
  };
  ...
  const handleEdit = () => {
    ...
    updateType(
      { id: editingRow.id, name: form.name.trim(), description: form.description.trim() },
      {
        ...
        onError: (err: any) => console.error('[ActivityTypes] update failed', err),
      },
    );
  };
  ...
  return (
    <>
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: '16px' } }}>
        <DialogTitle sx={{ fontWeight: 700 }}>Edit Activity Type</DialogTitle>
        <Divider />
        <DialogContent sx={{ pt: '20px !important' }}>{formContent}</DialogContent>
        <Divider />
        <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
