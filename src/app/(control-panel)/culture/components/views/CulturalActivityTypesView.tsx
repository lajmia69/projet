...
import {
	...
	useCreateCulturalActivityType,
	useUpdateCulturalActivityType,
	useDeleteCulturalActivityType
} from '../../api/hooks/useCultureProjectsActivities';
import { CulturalActivityType } from '../../api/types/projectsAndActivities';
...
function TypeRow({
	...
	onEdit: (t: CulturalActivityType) => void;
		...
		<>
			...
			>
				<Box sx={{
					...
					backgroundColor: 'secondary.main', display: 'flex',
					alignItems: 'center', justifyContent: 'center', flexShrink: 0
				}}>
					<FuseSvgIcon size={18} sx={{ color: 'secondary.contrastText' }}>lucide:sparkles</FuseSvgIcon>
				</Box>
			...
			<Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} maxWidth="xs" fullWidth
				...
				<DialogContent>
					<Typography variant="body2">
						<strong>{type.name}</strong> will be permanently deleted. Any activities using this type may be affected.
					</Typography>
				</DialogContent>
...
function TypeFormDialog({
	...
	open: boolean;
	initial: CulturalActivityType | null;
	onClose: () => void;
}) {
	const { mutate: create, isPending: creating } = useCreateCulturalActivityType();
	const { mutate: update, isPending: updating } = useUpdateCulturalActivityType();
	...
	return (
		...
		>
			<DialogTitle sx={{ fontWeight: 800 }}>{initial ? 'Edit Activity Type' : 'New Activity Type'}</DialogTitle>
			<Divider />
...
export default function CulturalActivityTypesView() {
	const { data: types = [], isLoading } = useCulturalActivityTypes();
	const { mutate: deleteType } = useDeleteCulturalActivityType();
	...
	const openEdit = (t: CulturalActivityType) => { setEditing(t); setDialogOpen(true); };
						...
						<div className="mx-auto flex w-full flex-col items-center justify-center">
							...
							<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.2 } }}>
								<Typography color="inherit" className="mt-2 text-center text-lg opacity-75">
									Manage the categories used when creating cultural activities.
								</Typography>
							</motion.div>
							<motion.div
							...
							>
								<FuseSvgIcon size={13} sx={{ color: 'rgba(255,255,255,0.7)' }}>lucide:sparkles</FuseSvgIcon>
					...
					<div className="mx-auto w-full max-w-2xl p-6 flex flex-col gap-4">
						...
						{filtered.length > 0 ? (
							...
							<div className="flex flex-col items-center gap-3 py-20">
								<FuseSvgIcon size={48} sx={{ color: 'text.disabled' }}>lucide:sparkles</FuseSvgIcon>
								<Typography color="text.secondary" variant="h6">No activity types yet</Typography>
								<Typography color="text.disabled" variant="body2">
									You need at least one type before you can create an activity.
								</Typography>
