...
import { useAccountId } from '../../api/hooks/useCultureProjectsActivities';
...
export default function CulturalProjectView() {
	...
	// Studio project link
	const { data: studioProject, refetch: refetchStudioProject } = useLinkedStudioProject('cultural_project', id);
	...
	const openEdit = () => {
		...
		setEditForm({
			...
			start_date: project.start_date, end_date: project.end_date ?? today,
	...
	const handleUpdate = () => {
		...
		const payload: UpdateCulturalProjectPayload = {
			id: project.id, name: editForm.name,
	...
	const handleOpenAudio = async () => {
		if (!studioProject && project?.id && project?.name && accountId > 0) {
	...
	return (
		<>
				...
				header={
					<div className="flex items-center gap-2 px-6 py-4 flex-wrap">
						...
						<div className="flex items-center gap-2 flex-wrap">
									...
									<Button
										...
										startIcon={
											creatingStudio
												...
												: <FuseSvgIcon size={15}>lucide:headphones</FuseSvgIcon>
										}
							...
							{canValidate && (
								<Button variant="contained" size="small" onClick={() => setConfirmValidate(true)}
							...
							{canPublish && (
								<Button variant="contained" size="small" onClick={() => setConfirmPublish(true)}
				...
				content={
					<div className="mx-auto w-full max-w-3xl p-6 flex flex-col gap-6">
						{canValidate && (
							<Paper sx={{ p: 2, borderRadius: '12px', display: 'flex', alignItems: 'center', gap: 2, backgroundColor: '#fefce8', border: '1px solid #fde047' }}>
								...
								<div className="flex-1 min-w-0">
									<Typography sx={{ fontWeight: 700, fontSize: '0.85rem', color: '#92400e' }}>This project is a draft</Typography>
							...
							<Paper sx={{ p: 2, borderRadius: '12px', display: 'flex', alignItems: 'center', gap: 2, backgroundColor: '#eff6ff', border: '1px solid #93c5fd' }}>
								...
								<div className="flex-1 min-w-0">
									...
									<Typography sx={{ fontSize: '0.78rem', color: '#1d4ed8' }}>
										This project has been approved. Click <strong>Publish</strong> to make it public.
									</Typography>
								</div>
						...
						<Paper sx={{ p: 3, borderRadius: '14px' }}>
							...
							<InfoRow icon="lucide:calendar"      label="Dates"     value={`${safeFormat(project.start_date)}${project.end_date ? ` → ${safeFormat(project.end_date)}` : ''}`} />
							<InfoRow icon="lucide:send"          label="Publishing" value={safeFormat(project.publishing_date)} />
									...
									<Button
										...
										startIcon={
											creatingStudio
												...
												: <FuseSvgIcon size={14}>lucide:plus</FuseSvgIcon>
										}
										sx={{ textTransform: 'none', fontWeight: 700 }}
									>
							...
							{allAudios.length === 0 ? (
								<div className="flex flex-col items-center gap-2 py-6">
									...
									<Typography color="text.disabled" variant="caption">
										{studioProject
											? 'Click "Add audio" to upload audio files to this project.'
									...
									{allAudios.map((audio) => (
										<div key={audio.id} className="flex items-center gap-3 rounded-lg border px-3 py-2"
											...
											{audio.format?.extension && (
												<Chip label={audio.format.extension.toUpperCase()} size="small"
			...
			<Dialog open={confirmValidate} onClose={() => setConfirmValidate(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '14px' } }}>
				...
				<DialogContent>
					<Typography variant="body2">Mark <strong>{project.name}</strong> as validated. It will still need to be published to become publicly visible.</Typography>
				</DialogContent>
			...
			<Dialog open={confirmPublish} onClose={() => setConfirmPublish(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '14px' } }}>
				...
				<DialogContent>
					<Typography variant="body2"><strong>{project.name}</strong> will be made publicly visible.</Typography>
				</DialogContent>
				<DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
			...
			<Dialog open={confirmDelete} onClose={() => setConfirmDelete(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '14px' } }}>
				...
				<DialogContent>
					<Typography variant="body2"><strong>{project.name}</strong> will be permanently deleted.</Typography>
				</DialogContent>
				<DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
