...
function Greeting({ name, t }: { name: string; t: (key: string) => string }) {
    const hour = new Date().getHours();
    const salutation = hour < 12 ? t('GOOD_MORNING') : hour < 18 ? t('GOOD_AFTERNOON') : t('GOOD_EVENING');
    return (
        <>
            <span style={{ color: 'rgba(232,228,218,0.55)' }}>{salutation}, </span>
            <span style={{ color: '#E8E4DA' }}>{name || t('GUEST_NAME')}</span>
        </>
    );
}
...
function QuickLinkCard({ link, delay, t }: { link: QuickLink; delay: number; t: (key: string) => string }) {
    return (
        <motion.div
            ...
            >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, p: 3, flex: 1, position: 'relative', zIndex: 1 }}>
                    ...
                    <Box sx={{ mt: 'auto', display: 'flex', alignItems: 'center', gap: 0.5 }}>
                        <Typography sx={{ fontSize: '0.75rem', fontWeight: 700, color: '#2D8B7C' }}>{t('OPEN')}</Typography>
...
export default function WelcomeView() {
    ...
    const quickLinks: QuickLink[] = [
        ...
        {
            label: t('RADIO_EMISSIONS'),
            description: t('RADIO_EMISSIONS_DESC'),
            icon: 'heroicons-outline:radio',
            url: '/content/radio/emissions'
        },
        {
            label: t('PODCAST'),
            description: t('PODCAST_DESC'),
            icon: 'heroicons-outline:microphone',
            url: '/content/podcast/courses'
        },
        {
            label: t('RADIO_EPISODES'),
            description: t('RADIO_EPISODES_DESC'),
            icon: 'heroicons-outline:play',
            url: '/content/radio/episodes'
        },
        {
            label: t('RADIO_REPORTAGES'),
            description: t('RADIO_REPORTAGES_DESC'),
                    ...
                    <Box sx={{ p: { xs: 3, md: 6 }, flex: 1 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
                            {quickLinks.map((link, i) => (
                                <QuickLinkCard key={link.url} link={link} delay={0.4 + i * 0.06} t={t} />
                            ))}
                        </div>
